...
let emptyBook = []
function firstBook (arr){
    for(let i = 0; i < arr.length; i ++){
        if(arr[i].title === "Things Fall Apart")
        return emptyBook.push(arr[i])
    }
}
firstBook(books)
console.log(emptyBook);


let newAuthor = []
function findAuthor (arr){
    for(let i = 0; arr.length; i++){
        if(arr[i].author === "Hans Christian Andersen")
        return newAuthor.push(arr[i])
    }
}
findAuthor(books)
